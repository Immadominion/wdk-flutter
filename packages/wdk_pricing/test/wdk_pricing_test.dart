...
void main() {
  group('BitfinexPricingClient', () {
    test('tickerFor uses the colon rule for symbols > 3 chars', () {
      final BitfinexPricingClient c =
          BitfinexPricingClient(httpClient: MockClient((_) async => http.Response('[]', 200)));
    ...
    test('getCurrentPrice posts to /calc/fx and returns data[0]', () async {
      ...
      final BitfinexPricingClient c = BitfinexPricingClient(
        httpClient: MockClient((http.Request req) async {
          ...
          expect(jsonDecode(req.body), <String, String>{'ccy1': 'BTC', 'ccy2': 'USD'});
    ...
    test('getMultiCurrentPrices parses last-price index 7 in input order', () async {
      final BitfinexPricingClient c = BitfinexPricingClient(
        httpClient: MockClient((http.Request req) async {
  ...
  group('PricingProvider caching', () {
    test('serves from cache within the duration, refetches after expiry', () async {
      int calls = 0;
