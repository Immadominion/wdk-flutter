...
import 'wdk_manager_messages.dart';
...
///
/// [sendOnly] marks a fire-and-forget command (the worklet sends no reply, e.g.
/// `log`, `dispose`). The request frame is identical to a normal call; the
/// client simply does not await a response.
class HrpcMethod {
  const HrpcMethod(
    this.command,
    this.encodeRequest,
...
class HrpcProtocol {
  ...
  static const String _mgr = '@wdk-core';

  /// The `wdkManager` (`@wdk-core`) worklet protocol. Command ids and body
  /// codecs are exact ports of the autogenerated HRPC spec shipped in
  /// `@tetherto/pear-wrk-wdk@1.0.0-beta.4` (`spec/hrpc/{hrpc.json,messages.js}`),
  /// the version `wdk-react-native-provider@beta.3` pins. Verified byte-for-byte
  /// against `tools/parity/wdk_manager_vectors.json`. `log`/`dispose` are
  /// send-only (no reply). Method names match the JS provider's call sites.
  factory HrpcProtocol.wdkManager() {
    HrpcMethod method(int command, String name, {bool sendOnly = false}) =>
        HrpcMethod(
          command,
          (Map<String, Object?> m) =>
              WdkManagerMessages.encode('$_mgr/$name-request', m),
          (Uint8List b) => WdkManagerMessages.decode('$_mgr/$name-response', b),
          sendOnly: sendOnly,
        );

    return HrpcProtocol(<String, HrpcMethod>{
      'log': method(0, 'log', sendOnly: true),
      'workletStart': method(1, 'workletStart'),
      'getAddress': method(2, 'getAddress'),
      'getAddressBalance': method(3, 'getAddressBalance'),
      'quoteSendTransaction': method(4, 'quoteSendTransaction'),
      'sendTransaction': method(5, 'sendTransaction'),
      'getAbstractedAddress': method(6, 'getAbstractedAddress'),
      'getAbstractedAddressBalance': method(7, 'getAbstractedAddressBalance'),
      'getAbstractedAddressTokenBalance': method(
        8,
        'getAbstractedAddressTokenBalance',
      ),
      'abstractedAccountTransfer': method(9, 'abstractedAccountTransfer'),
      'getApproveTransaction': method(10, 'getApproveTransaction'),
      'abstractedSendTransaction': method(11, 'abstractedSendTransaction'),
      'abstractedAccountQuoteTransfer': method(
        12,
        'abstractedAccountQuoteTransfer',
      ),
      'getTransactionReceipt': method(13, 'getTransactionReceipt'),
      'dispose': method(14, 'dispose', sendOnly: true),
    });
  }
}
...
///
/// Drive it with either [HrpcProtocol.secretManager] or
/// [HrpcProtocol.wdkManager] — both command tables and body codecs are
/// byte-exact ports of the pinned `pear-wrk-wdk@beta.4` /
/// `wdk-react-native-provider@beta.3` HRPC specs, verified against the real JS
/// encoders (see `tools/parity/README.md`).
class HrpcWorkletRpc implements WorkletRpc {
  ...
  Future<Map<String, Object?>> call(
    ...
    // Send-only commands (log/dispose) get the same request frame but no reply.
    if (spec.sendOnly) {
      _pipe.send(
