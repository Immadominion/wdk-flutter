...
void main() {
  test('WdkTheme.fromBrand keeps the brand color and varies by brightness', () {
    ...
    final WdkTheme dark = WdkTheme.fromBrand(
      primaryColor: brand,
      brightness: Brightness.dark,
    );
    final WdkTheme light = WdkTheme.fromBrand(
      primaryColor: brand,
  ...
  testWidgets('WdkAddressInput shows an injected validation error', (
    tester,
  ) async {
  ...
  testWidgets('WdkTransactionList renders rows and an empty state', (
    tester,
  ) async {
    await tester.pumpWidget(
      host(const WdkTransactionList(items: <WdkTxItem>[])),
    );
  ...
  testWidgets('WdkSeedPhrase hides words until revealed', (tester) async {
    await tester.pumpWidget(
      host(const WdkSeedPhrase(words: <String>['alpha', 'bravo', 'charlie'])),
    );
  ...
  testWidgets('WdkQrCode renders', (tester) async {
    await tester.pumpWidget(
      host(const WdkQrCode(data: 'bc1qexample', label: 'Scan')),
    );
